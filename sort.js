...
function partition(arr, begin, end) {
	...
	for (var i = begin; i < end; i++) {
		if (arr[i] < arr[pivot]) {
			var tmp = arr[count]; arr[count++] = arr[i]; arr[i] = tmp;
		}
	}
	var tmp = arr[count]; arr[count] = arr[pivot]; arr[pivot] = tmp;
	return count;
}
...
function heapSort (arr) {
	...
	for (var i = len - 1; i >= 0; i--) {
		var tmp = arr[i]; arr[i] = arr[0]; arr[0] = tmp;
		heapify(arr, i, 0);
	}
}
...
function heapify(arr, len, i) { 
	...
	if (right < len && arr[right] > arr[max]) 
		max = right;
	if (max != i) {
		var tmp = arr[i]; arr[i] = arr[max]; arr[max] = tmp;
		heapify(arr, len, max);
	}
}
