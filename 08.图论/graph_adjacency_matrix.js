...
class GraphAdjacencyMatrix {
  ...
  constructor(vertices, edges) {
    ...
    this.adjacencyMatrix = []; // 邻接矩阵，行列索引对应“顶点索引”
    // 添加顶点
  ...
  addVertex(val) {
    ...
    // 在邻接矩阵中添加一行
    this.adjacencyMatrix.push(Array(this.size() - 1).fill(0));
    // 在邻接矩阵中添加一列
    for (const row of this.adjacencyMatrix) row.push(0);
  }
  ...
  removeVertex(index) {
    ...
    // 在邻接矩阵中删除索引 index 的列
    for (const row of this.adjacencyMatrix) row.splice(index, 1);
  }
